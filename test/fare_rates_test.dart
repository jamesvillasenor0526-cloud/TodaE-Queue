...
void main() {
  ...
  group('what may be set as a rate', () {
    ...
    test('an ordinary change is read as given', () {
      final rates = FareRates.fromMap({'minimumFare': 40, 'ratePerKm': 12.5});
    ...
    test('a mistyped rate falls back, and only that one', () {
      ...
      // bad field should not throw away the other.
      final rates = FareRates.fromMap({'minimumFare': 1000, 'ratePerKm': 12});
    ...
    test('an old saved pick-up fee is ignored', () {
      // Saved before the fee was dropped; it was never charged.
      final rates = FareRates.fromMap({
        'minimumFare': 35,
        'ratePerKm': 10,
        'pickupFee': 15,
      ...
      expect(rates, FareRates.defaults);
      expect(rates.toMap().containsKey('pickupFee'), isFalse);
    });
    ...
    test('a round trip through a map keeps the rates', () {
      const set = FareRates(minimumFare: 45, ratePerKm: 13);
  ...
  group('what a ride costs at the rates in force', () {
    ...
    test('the out-of-town charge follows the per-km rate too', () {
      ...
      fare.rates = const FareRates(minimumFare: 35, ratePerKm: 15);
    ...
    test(
      ...
      () {
        ...
        expect(FareService.ratePerKm, 10);
      },
    );
  });
}
